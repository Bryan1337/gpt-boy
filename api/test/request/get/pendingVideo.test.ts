...
describe("pendingVideoRequest", () => {
	it("returns pending responses", async () => {
		...
		await pendingVideoRequest(request, response);

		expect(request.pages.soraPage.evaluate).toHaveBeenCalledWith(getPendingVideoResponse, {
			body: { taskId: "task-1" },
		});
